#!/usr/bin/env python3
"""
Hybrid Agents Demo - Combine original structure with working implementations
"""
...
def create_hybrid_agent(name, description, instruction):
    """Create a hybrid agent using original structure but direct Gemini API"""
    try:
        ...
        print(f"❌ Failed to create hybrid agent {name}: {e}")
        return None
...
def run_hybrid_agent(agent, prompt):
    """Run a hybrid agent with a prompt"""
    try:
        if not agent:
...
def demo_hybrid_agents():
    """Demo using hybrid agents with original structure but working implementation"""
    print("🚀 HYBRID AGENTS DEMO")
    print("=" * 80)
    ...
    try:
        # Create hybrid agents (original structure, working implementation)
        print("🔍 Creating hybrid agents...")
        
        congestion_agent = create_hybrid_agent(
        ...
        context_agent = create_hybrid_agent(
        ...
        print(f"✅ Successfully created {len(valid_agents)} hybrid agents!")
        for agent in valid_agents:
            print(f"   • {agent['name']}")
        
        # Test scenarios using hybrid agents
        scenarios = [
            {
        ...
        for scenario in scenarios:
            ...
            result = run_hybrid_agent(scenario['agent'], scenario['prompt'])
        ...
        if passed == total:
            ...
            print(f"✅ Hybrid agents are working perfectly!")
            ...
            print(f"\n🚀 FOR YOUR DEMO:")
            print(f"   • Hybrid approach: Original structure + working implementation")
            print(f"   • Real Gemini API calls are being made")
